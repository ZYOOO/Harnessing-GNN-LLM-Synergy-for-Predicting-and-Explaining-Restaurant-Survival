...
class Model:
    ...
    def __init__(self, vocab_size, args, user_vector, item_vector, char_vocab=0, pos_vocab=0,
    ...
    def _get_point_feed_dict(self, data, mode='training', lr=None):
        ...
        feed_dict = {
            ...
            self.q1_UniLM: data[self.imap['q1_UniLM']],
            self.q2_UniLM: data[self.imap['q2_UniLM']],
            self.q1_u_len: data[self.imap['q1_u_len']],
            self.q2_u_len: data[self.imap['q2_u_len']],
        }
        #
        ...
        if (mode != 'infer'):
            ...
            for i in range(len(data[self.imap['gen_outputs']])):
                padding_outputs.append(pad_to_max(
                    data[self.imap['gen_outputs']][i], max_len))
    ...
    def _joint_representation(self, q1_embed, q2_embed, q1_len, q2_len, q1_max,
        ...
        """
        logging.log(logging.INFO, "This is a _joint_representation log.")
        ...
        # print("******************************************************************")
        # print(np.shape(q1_output))
        # print(np.shape(self.user_batch))
        # print(np.shape(self.user_emd_batch))
        # print("******************************************************************")
        if self.args.implicit == 1:
            q1_output = tf.concat(
                [q1_output, self.user_batch], 1)
            q2_output = tf.concat(
                [q2_output, self.item_batch], 1)
    ...
    def _cal_key_loss(self, preds):
        ...
        indices = tf.concat(
            [tf.expand_dims(prefix, 2), tf.expand_dims(word, 2)], 2)
    ...
    def _beam_search_infer(self, q1_output, q2_output, r_input, reuse=None):
        ...
        with tf.variable_scope('gen_review', reuse=reuse) as scope:
            ...
            if r_input is not None:
                r_embed = tf.nn.embedding_lookup(
                    self.review_rating_embeddings, r_input)
                state = tf.nn.tanh(r_embed + tf.matmul(q1_output, self.review_user_mapping) +
            ...
            self.beam_batch_max = self.args.beam_size * \
                self.args.beam_size * self.args.batch_size

            #max_val = self.max_val
            #initializer = tf.random_uniform_initializer(-max_val, max_val, dtype=self.dtype)

            neg_words = tf.Variable(tf.constant(
                0.0, shape=[self.vocab_size]), trainable=False)
            neg_words = tf.reshape(tf.scatter_update(neg_words, tf.constant(
                self.UNK_tag), tf.constant(1.0)), shape=[1, self.vocab_size])
            ...
            eos_batch_max = tf.constant(
                self.EOS_tag, shape=[self.beam_batch_max])
            ...
            init_tag = tf.cast(
                tf.equal(tf.reshape(indices, shape=[self.beam_batch]), eos_batch), tf.int32)
            ...
            init_state = tf.reshape(tf.tile(state, [1, self.args.beam_size]), shape=[
                                    self.beam_batch, self.args.rnn_dim])
            ...
            def forward_one_step(end_tag, tag, answer, lens, loss, lm_inputs, state):
                ...
                # else:
                self.rnnlm_outputs, new_state = self.rnn_cell(
                    self.tip_inputs_embedded, state)

                #new_tag = 1 - tag
                loss_old_pad = tf.where(
                    tf.cast(tag, tf.bool), zero_batch, min_batch)
                loss_old = loss + loss_old_pad
                tag_old = tag
                lens_old = lens
                state_old = state
                answer_old = tf.concat(
                    [answer, tf.reshape(pad_batch, shape=[self.beam_batch, 1])], axis=1)

                loss_new_pad = tf.where(
                    tf.cast(tag, tf.bool), min_batch, zero_batch)
                loss_new = loss + loss_new_pad
                loss_new = tf.reshape(tf.tile(tf.reshape(loss_new, shape=[self.beam_batch, 1]), [
                                      1, self.args.beam_size]), shape=[self.beam_batch_max])
                ...
                answer_new = tf.reshape(tf.tile(answer, [1, self.args.beam_size]), shape=[
                                        self.beam_batch_max, -1])
                indices = tf.reshape(indices, shape=[self.beam_batch_max])
                answer_new = tf.concat([answer_new, tf.reshape(
                    indices, shape=[self.beam_batch_max, 1])], axis=1)

                state_new = tf.reshape(tf.tile(new_state, [1, self.args.beam_size]), shape=[
                                       self.beam_batch_max, -1])
                tag_new = tf.cast(tf.equal(indices, eos_batch_max), tf.int32)
                lens_new = tf.reshape(tf.tile(tf.reshape(lens, shape=[self.beam_batch, 1]), [
                                      1, self.args.beam_size]), shape=[self.beam_batch_max])
                lens_new = lens_new + 1

                # merge
                merge_tag = tf.concat([tf.reshape(tag_old, shape=[
                                      self.args.batch_size, -1]), tf.reshape(tag_new, shape=[self.args.batch_size, -1])], axis=1)
                merge_lens = tf.concat([tf.reshape(lens_old, shape=[
                                       self.args.batch_size, -1]), tf.reshape(lens_new, shape=[self.args.batch_size, -1])], axis=1)
                merge_state = tf.concat([tf.reshape(state_old, shape=[self.args.batch_size, self.args.beam_size, -1]), tf.reshape(
                    state_new, shape=[self.args.batch_size, self.args.beam_size * self.args.beam_size, -1])], axis=1)
                merge_loss = tf.concat([tf.reshape(loss_old, shape=[
                                       self.args.batch_size, -1]), tf.reshape(loss_new, shape=[self.args.batch_size, -1])], axis=1)
                #average_loss = tf.div(merge_loss, merge_lens)
                merge_answer = tf.concat([tf.reshape(answer_old, shape=[self.args.batch_size, self.args.beam_size, -1]), tf.reshape(
                    answer_new, shape=[self.args.batch_size, self.args.beam_size * self.args.beam_size, -1])], axis=1)
                merge_inputs = tf.concat([tf.reshape(lm_inputs, shape=[
                                         self.args.batch_size, -1]), tf.reshape(indices, shape=[self.args.batch_size, -1])], axis=1)

                merge_values, merge_indices = tf.nn.top_k(
                    merge_loss, self.args.beam_size)

                #new_loss = tf.reshape(merge_values, shape=[self.beam_batch])
                merge_indices = tf.reshape(
                    merge_indices, shape=[self.beam_batch, 1])
                range_batch = tf.reshape(tf.tile(tf.reshape(tf.range(self.args.batch_size), shape=[
                                         self.args.batch_size, 1]), [1, self.args.beam_size]), shape=[self.beam_batch, 1])
                index = tf.concat([range_batch, merge_indices], axis=1)

                new_loss = tf.reshape(tf.gather_nd(
                    merge_loss, index), shape=[self.beam_batch])
                new_tag = tf.reshape(tf.gather_nd(
                    merge_tag, index), shape=[self.beam_batch])
                new_lens = tf.reshape(tf.gather_nd(
                    merge_lens, index), shape=[self.beam_batch])
                new_state = tf.reshape(tf.gather_nd(
                    merge_state, index), shape=[self.beam_batch, -1])
                new_answer = tf.reshape(tf.gather_nd(
                    merge_answer, index), shape=[self.beam_batch, -1])
                new_inputs = tf.reshape(tf.gather_nd(
                    merge_inputs, index), shape=[self.beam_batch])
            ...
            sum_end, tag, answer, lens, loss, lm_inputs, state = tf.while_loop(condition, forward_one_step, [init_end_tag, init_tag, init_ans, init_len, init_loss, init_lm_inputs, init_state],  shape_invariants=[
                                                                               init_end_tag.get_shape(), init_tag.get_shape(), tf.TensorShape([self.beam_batch, None]), init_len.get_shape(), init_loss.get_shape(), init_lm_inputs.get_shape(), init_state.get_shape()])
    ...
    def _gen_review(self, q1_output, q2_output, r_input, reuse=None):
        ...
        with tf.variable_scope('gen_review', reuse=reuse) as scope:
            ...
            review_inputs_transpose = tf.transpose(
                self.gen_outputs, perm=[1, 0])
            #print (review_inputs_transpose.get_shape())
            #print (q1_output.get_shape())
            #print (state.get_shape())
            max_review_length = tf.reduce_max(self.gen_len)
            masks = tf.transpose(tf.sequence_mask(
                self.gen_len, maxlen=max_review_length, dtype=tf.float32), perm=[1, 0])
            ...
            init_accuracy = tf.reduce_sum(
                tf.cast(correct, tf.float32) * tf.gather(masks, 1))

            init_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=tf.gather(review_inputs_transpose, 1), logits=logits) * tf.gather(masks, 1))

            # tf.constant(0, dtype=tf.int32)#batch_dim#tf.cast(batch_dim, dtype=tf.float32) #tf.constant(self.batch_size, dtype=self.dtype)
            ...
            def forward_one_step(i, sum, loss, sum_key_loss, accuracy, state):
                ...
                # else:
                self.rnnlm_outputs, new_state = self.rnn_cell(
                    self.review_inputs_embedded, state)

                # dropout before output layer
                self.dropout_out = tf.nn.dropout(
                    self.rnnlm_outputs, self.dropout)
                # if self.use_strong_feeding:
                ...
                correct = tf.equal(castm, tf.gather(
                    review_inputs_transpose, i+1))
                accuracy = accuracy + \
                ...
                loss = loss + tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
                    labels=tf.gather(review_inputs_transpose, i+1), logits=logits) * tf.gather(masks, i+1))
            ...
            iteration, sum, loss, sum_key_loss, accuracy, state = tf.while_loop(condition, forward_one_step, [
                                                                                init_iteration, init_sum, init_loss, init_sum_key_loss, init_accuracy, state])
    ...
    def prepare_hierarchical_input(self):
        ...
        """
        q1_UniLM, self.qmax = clip_sentence(self.q1_UniLM, self.q1_u_len)
        q2_UniLM, self.a1max = clip_sentence(self.q2_UniLM, self.q2_u_len)
        ...
        q2_embed, q2_len = hierarchical_flatten(q2_embed,
                                                ...
                                                self.args.smax)
        # print(np.shape(q1_embed))
        # print(np.shape(q1_UniLM))
        # q3_embed, q3_len = hierarchical_flatten(q3_embed,
        #                                         self.q3_len,
        #                                         self.args.smax)
        # q1_UniLM, q1_u_len = hierarchical_flatten(q1_UniLM,
        #                                         self.q1_u_len,
        #                                         self.args.smax)
        # q2_UniLM, q2_u_len = hierarchical_flatten(q2_UniLM,
        #                                         self.q2_u_len,
        #                                         self.args.smax)
        # print(q1_len)

        # self.o1_embed = q1_UniLM
        # self.o2_embed = q2_UniLM
        # self.o3_embed = q3_embed
        # self.o1_len = q1_u_len
        # self.o2_len = q2_u_len
        # self.o3_len = q3_len
        ...
        if self.args.masking == 1:
            ...
            #                                      reuse=True, pool=True,
            #                                      name='sent', mask=None)
        # _, q1_UniLM = self.learn_single_repr(q1_UniLM, q1_u_len, self.args.smax,
        #                                          self.args.base_encoder,
        #                                          reuse=True, pool=True,
        #                                          name='sent', mask=None)
        # _, q2_UniLM = self.learn_single_repr(q2_UniLM, q2_u_len, self.args.smax,
        #                                          self.args.base_encoder,
        #                                          reuse=True, pool=True,
        #                                          name='sent', mask=None)

        _dim = q2_UniLM.get_shape().as_list()[1]
        q1_embed = tf.reshape(q1_embed, [-1, self.args.dmax, _dim])
        q2_embed = tf.reshape(q2_embed, [-1, self.args.dmax, _dim])
        # q1_UniLM = tf.reshape(q1_UniLM, [-1, self.args.dmax, _dim])
        # q2_UniLM = tf.reshape(q2_UniLM, [-1, self.args.dmax, _dim])
        # q3_embed = tf.reshape(q3_embed, [-1, self.args.dmax, _dim])
        self.q1_embed = q1_embed
        self.q2_embed = q2_embed
        # print(np.shape(self.q1_embed))
        # self.q1_UniLM = q1_UniLM
        # self.q2_UniLM = q2_UniLM
    ...
    def prepare_hierarchical_input2(self):
        """ Supports hierarchical data input
        Converts word level -> sentence level
        """
        q1_inputs, self.qmax = clip_sentence(self.q1_inputs, self.q1_len)
        ...
        # This is found in nn.py in tylib
        print("Hierarchical Flattening")
        self.q1_UniLM = tf.convert_to_tensor(
            self.q1_UniLM)
        self.q2_UniLM = tf.convert_to_tensor(
            self.q2_UniLM)

        self.q1_UniLM = tf.to_float(self.q1_UniLM)
        self.q2_UniLM = tf.to_float(self.q2_UniLM)

        q1_embed = self.q1_UniLM
        q2_embed = self.q2_UniLM
        q1_len = self.q1_u_len
        q2_len = self.q2_u_len
        # q1_len = self.q1_len
        # q2_len = self.q2_len

        # print("===================unilm==========================")
        # print(np.shape(self.q1_UniLM))
        ...
        # print("===================learn_single_repr==========================")
        # print(np.shape(q2_embed))

        # _dim = q1_embed.get_shape().as_list()[1]
        q1_embed = tf.reshape(q1_embed, [-1, self.args.dmax, 50])
        q2_embed = tf.reshape(q2_embed, [-1, self.args.dmax, 50])

        # print("===================learn_single_repr2==========================")
        # print(np.shape(q2_embed))
        # q3_embed = tf.reshape(q3_embed, [-1, self.args.dmax, _dim])
        self.q1_embed = q1_embed
        self.q2_embed = q2_embed
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('q2_input'):
                self.q2_inputs = tf.placeholder(tf.int32, shape=[None,
                                                                 self.args.amax],
                                                name='q2_inputs')
            with tf.name_scope('q1_UniLM'):
                self.q1_UniLM = tf.placeholder(tf.float32, shape=[None,
                                                                  self.args.qmax],
                                               name='q1_UniLM')
            with tf.name_scope('q2_UniLM'):
                self.q2_UniLM = tf.placeholder(tf.float32, shape=[None,
                                                                  self.args.amax],
                                               name='q2_UniLM')
            ...
            with tf.name_scope('q1_u_lengths'):
                self.q1_u_len = tf.placeholder(tf.int32, shape=len_shape)
            with tf.name_scope('q2_u_lengths'):
                self.q2_u_len = tf.placeholder(tf.int32, shape=len_shape)
            ...
            if self.args.heterougenous == 1:
                ...
                with tf.name_scope('Environment_id'):
                    self.Environment_id = tf.placeholder(
                        tf.int32, shape=[None])
            ...
            with tf.variable_scope('embedding_layer'):
                ...
                if self.args.implicit == 1:
                    ...
                    # self.user_emb = tf.convert_to_tensor(
                    #     self.user_vector)
                    # self.item_emb = tf.convert_to_tensor(
                    #     self.item_vector)
                    #
                    # self.item_emb = tf.to_float(self.item_emb)
                    # self.user_emb = tf.to_float(self.user_emb)
                    ...
                    # self.user_emd_batch = tf.nn.embedding_lookup(self.user_emb,
                    #                                              self.item_id)
                    #
                    # self.item_emd_batch = tf.nn.embedding_lookup(self.item_emb,
                    #                                              self.item_id)
            ...
            if(self.mode == 'HREC' and self.args.base_encoder != 'Flat'):
                # Hierarchical mode
                self.prepare_hierarchical_input2()
                q1_len = tf.cast(tf.count_nonzero(self.q1_u_len, axis=1),
                                 tf.int32)
                q2_len = tf.cast(tf.count_nonzero(self.q2_u_len, axis=1),
                                 tf.int32)
            ...
            # self.q1_UniLM = tf.to_float(self.q1_UniLM)
            # self.q2_UniLM = tf.to_float(self.q2_UniLM)
            # _dim = self.q1_UniLM.get_shape().as_list()[1]
            # self.q1_UniLM  = tf.reshape(self.q1_UniLM, [-1, self.args.dmax, _dim])
            # self.q2_UniLM  = tf.reshape(self.q2_UniLM, [-1, self.args.dmax, _dim])
            ...
            with tf.name_scope("train"):
                with tf.name_scope("cost_function"):
                    ...
                    if self.args.feed_rating == 0:
                        ...
                        r_input = tf.clip_by_value(
                            tf.cast(tf.reshape(self.output_pos, [-1]), dtype=tf.int32), 1, 5) - 1

                    with tf.name_scope('generation_results'):
                        self.gen_results = self._beam_search_infer(
                            q1_output, q2_output, r_input)

                    with tf.name_scope('generation_loss'):
                        #self.gen_loss, self.gen_acc = self._gen_review(q1_output, q2_output)
                        self.gen_loss, self.gen_acc, self.key_word_loss = self._gen_review(
                            q1_output, q2_output, r_input)
                ...
                self.grads = _none_to_zero(
                    tf.gradients(self.cost, tvars), tvars)
